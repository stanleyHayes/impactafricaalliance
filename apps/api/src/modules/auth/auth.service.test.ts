...
describe('AuthService.login', () => {
  ...
  let tokens: { issueTokens: ReturnType<typeof vi.fn> };
  let passwords: { compare: ReturnType<typeof vi.fn> };
  let service: AuthService;

  beforeEach(() => {
    users = { findByEmail: vi.fn(), findById: vi.fn() };
    tokens = { issueTokens: vi.fn().mockReturnValue({ accessToken: 'a', refreshToken: 'r' }) };
